...
const CategoryProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/products/?category__slug=${category}`);
        const fetchedProducts = response.data;
        setProducts(response.data);
        if (fetchedProducts.length > 0 && fetchedProducts[0].category.length > 0) {
